...
public class Q8 {
    ...
    public static void main(String[] args) {
        Cake[] cakeShelf = new Cake[10];
        cakeShelf[0] = new Danish(true, 2, 100, "None", 10.0, 60);
        cakeShelf[1] = new FrostedCake(true, 3, 150, "Sprinkles", "Chocolate", true);
        cakeShelf[2] = new FrostedCake(false, 4, 120, "None", "Vanilla", false);
        int count = 0;
        for (Cake c : cakeShelf) {
            if (c != null) count++;
        }
        System.out.println("Total cakes on shelf: " + count);
        count = 0;
        for (Cake c : cakeShelf) {
            if (c instanceof Danish)
                count++;
        }
        System.out.println("Total Danish cakes: " + count);
        count = 0;
        for (Cake c : cakeShelf) {
            if (c instanceof FrostedCake)
                count++;
        }
        System.out.println("Total Frosted cakes: " + count);
        FrostedCake c1;
        Danish c2;
        c2 = null;
        for (Cake c : cakeShelf) {
